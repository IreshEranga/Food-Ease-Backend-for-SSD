...
exports.login = async (req, res) => {
  try {
    ...
    if (userType === "customer" && user.address) {
      responseUser.address = user.address;
      responseUser.profileComplete = user.profileComplete;
    }
    if (userType === "rider" && user.vehicleNumber) {
      responseUser.vehicleNumber = user.vehicleNumber;
      responseUser.isAvailable = user.isAvailable;
...
exports.googleLoginCallback = async (req, res) => {
  try {
    const user = req.user; // User set by passport
    if (!user) {
      return res.redirect(`${process.env.FRONTEND_URL}/login?error=auth_failed`);
    }

    const token = generateToken(user.customerID, user.roleID);
    ...
    if (user.profilePicture) {
      responseUser.profilePicture = user.profilePicture;
    }
